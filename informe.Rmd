...
Visualización de los post-estratos con las variables auxiliares para ver si sirven para explicar la variable de interés.
...
```{r}
muestra = muestra %>% mutate(g_rake1=weights(rake1)/w_nr_post)
```
...
```{r}
muestra = muestra %>% mutate(g_r2=weights(r2)/w_nr_post)
```
...
design_final %>%
  filter(activo==1) %>% 
  as.svrepdesign(design=., type="subbootstrap", replicates=500) %>% 
  svymean(~desocupado, .)

```

Con la estimación del error estándar realizada por el método del último conglomerado, el SE estimado es 0.00338, mientras que el SE estimado a través de Bootstrap Rao Wu es 0.0032.
...
boot_ing_prom <- as.svrepdesign(design=design_final, type="subbootstrap", replicates=500) #Para las estimaciones del SE con Bootstrap Rao Wu

#Estimación del SE con Bootstrap Rao Wu
svymean(~ingreso, design=boot_ing_prom)
```
